...
var require_find_pid = __commonJS({
  "../../node_modules/.pnpm/find-process@1.4.7/node_modules/find-process/lib/find_pid.js"(exports2, module2) {
    ...
    var ensureDir = (path4) => new Promise((resolve, reject) => {
      if (fs3.existsSync(path4)) {
        resolve();
      } else {
        fs3.mkdir(path4, (err2) => {
    ...
    var finders = {
      ...
      android(port) {
        return new Promise((resolve, reject) => {
          ...
          ensureDir(dir).then(() => {
            utils.exec(cmd, () => {
              fs3.readFile(file, "utf8", (err2, data) => {
                fs3.unlink(file, () => {
                });
                if (err2) {
                  reject(err2);
...
var require_find_process = __commonJS({
  "../../node_modules/.pnpm/find-process@1.4.7/node_modules/find-process/lib/find_process.js"(exports2, module2) {
    ...
    function fetchBin(cmd) {
      const pieces = cmd.split(path4.sep);
      ...
      return fixed.join(path4.sep);
    }
    function fetchName(fullpath) {
      if (process.platform === "darwin") {
        const idx = fullpath.indexOf(".app/");
        if (idx >= 0) {
          return path4.basename(fullpath.substring(0, idx));
        }
      }
      return path4.basename(fullpath);
    }
    var finders = {
      darwin(cond) {
        return new Promise((resolve, reject) => {
...
async function shopifyDevelopersFqdn() {
  const environment = serviceEnvironment();
...
// ../../node_modules/.pnpm/atomically@2.0.1/node_modules/atomically/dist/index.js
init_cjs_shims();
import path2 from "node:path";

// ../../node_modules/.pnpm/stubborn-fs@1.2.4/node_modules/stubborn-fs/dist/index.js
init_cjs_shims();
import fs from "node:fs";
import { promisify } from "node:util";

// ../../node_modules/.pnpm/stubborn-fs@1.2.4/node_modules/stubborn-fs/dist/attemptify.js
init_cjs_shims();
...
// ../../node_modules/.pnpm/stubborn-fs@1.2.4/node_modules/stubborn-fs/dist/constants.js
init_cjs_shims();
...
// ../../node_modules/.pnpm/stubborn-fs@1.2.4/node_modules/stubborn-fs/dist/handlers.js
init_cjs_shims();
var Handlers = {
  /* API */
...
// ../../node_modules/.pnpm/stubborn-fs@1.2.4/node_modules/stubborn-fs/dist/retryify.js
init_cjs_shims();

// ../../node_modules/.pnpm/stubborn-fs@1.2.4/node_modules/stubborn-fs/dist/retryify_queue.js
init_cjs_shims();
...
// ../../node_modules/.pnpm/atomically@2.0.1/node_modules/atomically/dist/constants.js
init_cjs_shims();
...
// ../../node_modules/.pnpm/atomically@2.0.1/node_modules/atomically/dist/utils/lang.js
init_cjs_shims();
...
// ../../node_modules/.pnpm/atomically@2.0.1/node_modules/atomically/dist/utils/scheduler.js
init_cjs_shims();

// ../../node_modules/.pnpm/atomically@2.0.1/node_modules/atomically/dist/utils/temp.js
init_cjs_shims();
...
// ../../node_modules/.pnpm/when-exit@2.1.0/node_modules/when-exit/dist/node/constants.js
init_cjs_shims();
...
// ../../node_modules/.pnpm/atomically@2.0.1/node_modules/atomically/dist/index.js
function writeFileSync(filePath, data, options2 = DEFAULT_WRITE_OPTIONS) {
  if (isString(options2))
    return writeFileSync(filePath, data, { encoding: options2 });
  const timeout = Date.now() + ((options2.timeout ?? DEFAULT_TIMEOUT_SYNC) || -1);
  ...
  try {
    ...
    fd = dist_default.retry.openSync(timeout)(tempPath, "w", options2.mode || DEFAULT_FILE_MODE);
    if (options2.tmpCreated) {
      options2.tmpCreated(tempPath);
    }
    if (isString(data)) {
      dist_default.retry.writeSync(timeout)(fd, data, 0, options2.encoding || DEFAULT_ENCODING);
    ...
    if (options2.chown && (options2.chown.uid !== DEFAULT_USER_UID || options2.chown.gid !== DEFAULT_USER_GID)) {
      dist_default.attempt.chownSync(tempPath, options2.chown.uid, options2.chown.gid);
    }
    if (options2.mode && options2.mode !== DEFAULT_FILE_MODE) {
      dist_default.attempt.chmodSync(tempPath, options2.mode);
    }
    try {
...
init_cjs_shims();
var debounceFn = (inputFunction, options2 = {}) => {
...
var Conf = class {
  constructor(partialOptions = {}) {
    ...
    if (!options2.cwd) {
      if (!options2.projectName) {
        throw new Error("Please specify the `projectName` option.");
      }
      options2.cwd = envPaths(options2.projectName, { suffix: options2.projectSuffix }).config;
    }
    __classPrivateFieldSet(this, _Conf_options, options2, "f");
    if (options2.schema) {
      if (typeof options2.schema !== "object") {
      ...
      const schema = {
        type: "object",
        properties: options2.schema
      };
      __classPrivateFieldSet(this, _Conf_validator, ajv.compile(schema), "f");
      for (const [key, value] of Object.entries(options2.schema)) {
        if (value?.default) {
    ...
    if (options2.serialize) {
      this._serialize = options2.serialize;
    }
    if (options2.deserialize) {
      this._deserialize = options2.deserialize;
    }
    this.events = new EventEmitter();
    __classPrivateFieldSet(this, _Conf_encryptionKey, options2.encryptionKey, "f");
    const fileExtension = options2.fileExtension ? `.${options2.fileExtension}` : "";
    this.path = path3.resolve(options2.cwd, `${options2.configName ?? "config"}${fileExtension}`);
    const fileStore = this.store;
    const store2 = Object.assign(createPlainObject(), options2.defaults, fileStore);
    ...
    if (options2.migrations) {
      if (!options2.projectVersion) {
        throw new Error("Please specify the `projectVersion` option.");
      }
      this._migrate(options2.migrations, options2.projectVersion, options2.beforeEachMigration);
    }
  }
...
async function validateRedirectionPortAvailability(port) {
  const { killPortProcess } = await import("./lib-2EBQB76J.js");
...
async function graphqlRequest(options2) {
  const { query, api, url: url2, token, addedHeaders, variables, responseOptions } = options2;
  const headers = {
    ...addedHeaders,
    ...buildHeaders(token)
  };
